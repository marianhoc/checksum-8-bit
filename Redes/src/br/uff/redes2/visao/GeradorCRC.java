...
public class GeradorCRC {
    ...
    public void executa() {
        ...
        for (int i = 0; i < quantidadeIteracoes; i++) {
            pacoteLocal = this.gerarMensagemDeTamanho(tamanhoPacote);

            if (debug) {
                System.out.println("PacoteLocal: " + pacoteLocal);
            }

            mensagemCRCOriginal = new MensagemCRC(pacoteLocal, polinomio);
            mensagemCRCAlterada = new MensagemCRC(pacoteLocal, polinomio);
    ...
    public void relatorio() {
        ...
        System.out.println("Percentual de colisões: " + this.quantidadeColisoes / this.quantidadeIteracoes + "%");
    }
     ...
     *
     */
    private String gerarMensagemDeTamanho(long bytes) {
